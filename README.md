...
If the target library file already exists, new symbols are appended to it by default. If the same symbol is already present, use `--overwrite` to replace it.
...
Custom symbol properties can be added with repeatable `--custom-field KEY:VALUE` flags. These properties are written only to KiCad v6 symbol libraries.

```bash
easyeda2kicad --symbol --lcsc_id=C2040 --custom-field "Manufacturer:Texas Instruments" --custom-field "Mfr. Part #:SN74LVC1G125DBVR" --custom-field "LCSC ID:C2040" --custom-field "Package:SOT-23-5"
```
